...
The baseline calibration intentionally uses strong anchoring ($\kappa=0.005$) and moderate impact ($\lambda=0.20$), resulting in modest overconfidence effects on mispricing ($|\Delta \E[|p-v|]| \approx 0.004$). This subsection distinguishes two notions: (i) \emph{level amplification}---large mispricing/volatility levels driven by weak anchoring and/or high noise trading; and (ii) \emph{overconfidence (treatment) effects}---incremental differences when increasing $k$ from $1$ to $3$. The numerical results (Section~\ref{sec:meaningful_regions}) show substantial level amplification under stress regimes, while $k$-effects on mispricing remain modest across parameterizations. The following proposition gives an analytical mechanism for why $k$ predominantly affects disagreement rather than mispricing.
...
\paragraph{Level amplification from market microstructure.}
Weak anchoring ($\kappa < 0.002$) and/or high noise trading ($\phi > 0.75$) can generate large mispricing and volatility \emph{levels} (e.g., $\E[|p-v|]$ exceeding $3$--$9$ vs.\ $\approx 1.9$ in baseline). This level amplification is primarily driven by $(\kappa,\phi)$ rather than the overconfidence parameter $k$: within stress cases, outcomes are nearly unchanged between $k=1$ and $k=3$ (Table~\ref{tab:mispricing_stress}).

\paragraph{Overconfidence effects: primarily on disagreement and trading intensity.}
In both baseline and stress calibrations, increasing $k$ from $1$ to $3$ primarily increases cross-sectional disagreement ($\Delta \E[\sigma_i(\hat v_i)] \approx +0.059$) and trading intensity, while effects on mispricing levels remain modest ($|\Delta \E[|p-v|]| \approx 0.004$ in baseline, similarly small in stress cases).
Proposition~\ref{prop:mechanism_k} provides the analytical reason: $k$ enters through perceived signal precision (equivalently, the perceived measurement variance $R'(k)$ and the Kalman gain), which increases the dispersion of individual beliefs and hence trading intensity.
But the mean-field closure depends only on the cross-sectional mean belief $m_t$, and under our information structure the mean-belief tracking error variance is not amplified in $k$ (it is decreasing in $k$), so equilibrium mispricing responds only weakly.
...
These extensions are implemented in the codebase (Section~\ref{sec:meaningful_regions}) but are not the focus of the current numerical results, which demonstrate level amplification from $(\kappa,\phi)$ rather than $k$-driven amplification.

\paragraph*{Scope and interpretation.}
Proposition~\ref{prop:ct_cara_lqg} relies on our price-taking mean-field specification and on the simplification that belief updating conditions only on the private signal (Appendix~A). In the particle simulations we implement a discrete-time mean-field consistent dynamics: at each step we compute $(\bar x_t,\bar u_t)$ from the scalar stock--flow fixed point in Proposition~\ref{prop:step_fixed_point}, with uniqueness ensured by the per-step contraction condition $0\le B_t<\Delta t$ (Theorem~\ref{thm:contraction}). Equation \eqref{eq:mean_field_closure} is retained only as a transparent position-impact benchmark (not used in baseline simulations).
