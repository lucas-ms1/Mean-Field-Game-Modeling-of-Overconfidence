...
\subsection{Price-Augmented Filtering (Robustness Variant)}
 ...
 $R_p=\phi^2/(\kappa^2\Delta t)$ (scaled by a factor $\sigma_{p,\mathrm{obs}}^2$ in the code to probe robustness). We then apply an additional Kalman update using $\tilde v^{(p)}_t$ as a measurement (after processing the private signal), holding the state prediction fixed.
